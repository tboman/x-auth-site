...
| `ADMIN_EMAILS` | _(unset)_ | Comma-separated Google-account allowlist for the staff `/admin` console (all-tenants view). Empty denies all staff sign-ins. Customer self-service signup and the tenant-owner dashboard are unaffected. Seeds each address as a staff user with the `administrator` role on boot. |
